...
export const Pagination = forwardRef<HTMLElement, PaginationProps>(({
  ...
  // Show search when searchable and original total items would warrant pagination
  const originalTotalPages = totalItems && itemsPerPage ? Math.ceil(totalItems / itemsPerPage) : totalPages;
  const shouldShowSearch = searchable && originalTotalPages > 1;

  // Don't render if there's only one page or no pages (unless search is enabled)
  if (totalPages <= 1 && !shouldShowSearch) return null;

  // Calculate info for display
  const getInfoText = () => {
    if (!showInfo || !totalItems || !itemsPerPage) return null;

    const currentTotalItems = filteredItems ?? totalItems;
    const startItem = (currentPage - 1) * itemsPerPage + 1;
    const endItem = Math.min(currentPage * itemsPerPage, currentTotalItems);

    if (searchQuery && filteredItems !== undefined && filteredItems !== totalItems) {
      return `Showing ${startItem}-${endItem} of ${filteredItems} filtered results (${totalItems} total)`;
    }

    return `Showing ${startItem}-${endItem} of ${currentTotalItems} results`;
  };
  ...
  return (
    <div className="space-y-3">
      ...
      {/* Results Summary */}
      {shouldShowSearch && searchQuery.trim() && filteredItems !== undefined && (
        <div className="text-center">
          <span className={clsx(paginationTokens.info.muted, typography.text.sm)}>
            {filteredItems} of {totalItems} items matching &quot;{searchQuery}&quot;
          </span>
        </div>
      )}
