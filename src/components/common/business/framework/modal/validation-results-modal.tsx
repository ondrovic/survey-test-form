...
export const ValidationResultsModal: React.FC<ValidationResultsModalProps> = ({
    ...
    const parseErrors = (errors: string[]): ValidationError[] => {
        ...
        return errors.map(error => {
            ...
            const mustReferenceMatch = error.match(/([\w\s]+) field must reference a ([\w\s-]+)/);
            ...
            if (configMatch && sectionMatch && fieldMatch && mustReferenceMatch) {
                ...
                const requiredItemType = mustReferenceMatch[2].toLowerCase().replace(/\s+/g, '-'); // "select-option-set"
                ...
                return {
                    ...
                    missingItemType: requiredItemType as ValidationError['missingItemType'],
    ...
    const handleCreateAllMissingItems = async () => {
        try {
            ...
            for (const error of validationErrors) {
                const canCreate = error.missingItemId && error.fieldType !== 'Section Configuration' && !error.fieldType?.includes('field configuration');
                ...
                if (canCreate && !createdItems.has(error.missingItemId)) {
                    ...
                    console.log('⏭️ Skipping item that cannot be auto-created or already created:', error.missingItemId || 'structural-error');
                }
            }
                ...
                <div className="flex-1 overflow-y-auto p-6">
                    {hasErrors ? (
                        <div className="space-y-6">
                            ...
                            {/* Detailed Error List */}
                            <div>
                                <h4 className="text-lg font-medium text-gray-900 mb-4">Validation Issues</h4>
                                <div className="space-y-3">
                                    {validationErrors.map((error, index) => {
                                        const canCreate = error.missingItemId && error.missingItemType !== 'radio-option-set' || error.missingItemId;
                                        const isStructuralError = error.fieldLabel.includes('Section has no fields') || error.fieldType === 'Section Configuration';
                                        const isConfigurationError = error.fieldType?.includes('field configuration') && !error.missingItemId;
                                        ...
                                        return (
                                            <div key={index} className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                                                <div className="flex items-start justify-between">
                                                    <div className="flex-1">
                                                        <div className="flex items-center mb-2">
                                                            <span className="text-2xl mr-3">
                                                                {isStructuralError || isConfigurationError ? '⚠️' : getMissingItemIcon(error.missingItemType)}
                                                            </span>
                                                            <div>
                                                                ...
                                                                <p className="text-sm text-gray-600">
                                                                    {isStructuralError 
                                                                        ...
                                                                        : isConfigurationError
                                                                        ? `Field "${error.fieldLabel}" in ${error.configTitle} > ${error.sectionTitle}${error.subsectionTitle ? ` > ${error.subsectionTitle}` : ''} needs to be configured with a ${getMissingItemDisplayName(error.missingItemType).toLowerCase()}`
                                                                        : `Referenced by field "${error.fieldLabel}" in ${error.configTitle} > ${error.sectionTitle}${error.subsectionTitle ? ` > ${error.subsectionTitle}` : ''}`
                                                                    }
                                                                </p>
                                                            </div>
                                                        </div>
                                                    ...
                                                    {canCreate && !isStructuralError && !isConfigurationError && (
                                                        <Button
                                                            variant="outline"
                                                            size="sm"
                                                    ...
                                                    {(isStructuralError || isConfigurationError) && (
                                                        <div className="ml-4 text-sm text-amber-600 bg-amber-50 px-3 py-1 rounded">
                                                            Manual fix required
                                                        </div>
                                                    )}
                                                </div>
                            ...
                            <div className="flex justify-end space-x-3 pt-4 border-t border-gray-200">
                                ...
                                {validationErrors.some(error => error.missingItemId && error.fieldType !== 'Section Configuration' && !error.fieldType?.includes('field configuration')) && (
                                    <Button
                                        variant="primary"
