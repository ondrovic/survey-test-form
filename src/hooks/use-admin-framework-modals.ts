...
export const useAdminFrameworkModals = () => {
  ...
  const handlersRef = useRef<{
    ...
    onSaveInstanceSettings?: (data: SurveyInstance, updates: any) => Promise<void>;
  ...
  const modalActions = {
    deleteModal: {
      open: (data: DeleteModalData) => {
        ...
        showConfirmation({
          ...
          onConfirm: () => {
            if (currentDeleteData.current && handlersRef.current.onConfirmDelete) {
          ...
          onAlternative: data.type === 'instance' ? () => {
            if (currentDeleteData.current && handlersRef.current.onDeactivateInstance) {
              handlersRef.current.onDeactivateInstance(currentDeleteData.current.id, currentDeleteData.current.name);
              currentDeleteData.current = null;
            }
          } : undefined
        });
    ...
    settingsModal: {
      open: (data: SurveyInstance) => {
        ...
        import('@/components/common/framework7').then(({ InstanceSettingsModal }) => {
          openModal(
            'instance-settings',
            React.createElement(InstanceSettingsModal, {
              ...
              onSave: async (updates) => {
                if (currentSettingsData.current && handlersRef.current.onSaveInstanceSettings) {
                  await handlersRef.current.onSaveInstanceSettings(currentSettingsData.current, updates);
                  currentSettingsData.current = null;
                }
              }
            })
    ...
    createInstanceModal: {
      open: (data: SurveyConfig) => {
        ...
        import('@/components/admin/framework/modals').then(({ CreateInstanceModal }) => {
          openModal(
            'create-instance',
            React.createElement(CreateInstanceModal, {
              config: data,
              existingInstances: handlersRef.current.surveyInstances?.filter(instance => instance.configId === data.id) || [],
              onClose: () => {
                currentCreateData.current = null;
                closeModal('create-instance');
              },
              onConfirm: () => {
                if (currentCreateData.current && handlersRef.current.onConfirmCreateInstance) {
                  handlersRef.current.onConfirmCreateInstance(currentCreateData.current);
                  currentCreateData.current = null;
                }
              }
            })
